...
info(){
    ...
    vps=ssr
    wget --no-check-certificate -O trojan_tmpl.html https://raw.githubusercontent.com/JeannieStudio/all_install/master/trojan_tmpl.html
    chmod +x trojan_tmpl.html
    eval "cat <<EOF
    $(< trojan_tmpl.html)
    EOF
    "  > trojan.html
    cp /root/trojan.html /var/www/trojan.html
